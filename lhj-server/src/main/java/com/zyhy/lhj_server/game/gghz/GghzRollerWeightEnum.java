 ...
 * 轮子权重
 */
public enum GghzRollerWeightEnum {
	...
	private GghzRollerWeightEnum(int id, int index, GghzIconEnum icon, int weight){
		this.id = id;
		this.index = index;
		this.icon = icon;
		this.weight = weight;
	 ...
	 * @return
	 */
	public static Collection<? extends GghzWindowInfo> windowInfo(int c, List<GghzOdds> all) {
		List<GghzWindowInfo> ws = new ArrayList<GghzWindowInfo>();
		List<GghzRollerWeightEnum> rollers = new ArrayList<>();
		List<GghzOdds> col = new ArrayList<>();
		List<IconInfo> icons = new ArrayList<>();
		// 取出一列的图标
		for (GghzOdds odds : all) {
			if(odds.getCol() == c){
				col.add(odds);
			}
		}
		//System.out.println("col" + col);
		for (GghzOdds odds : col) {
			GghzIconEnum icon = null;
			for (GghzIconEnum e : GghzIconEnum.values()) {
		...
		for (GghzRollerWeightEnum roller : values()) {
			if (roller.getId() == c) {
				rollers.add(roller);
			}
		}
		
		for (GghzRollerWeightEnum roller : rollers) {
		...
		int total = 0;
		for(GghzRollerWeightEnum e : rollers){
			total += e.getWeight();
		}
		int num = RandomUtil.getRandom(0, total);
		int v = 0;
		GghzRollerWeightEnum roller1 = null;
		GghzRollerWeightEnum roller2 = null;
		GghzRollerWeightEnum roller3 = null;
		Iterator<GghzRollerWeightEnum> it = rollers.iterator();
		while(it.hasNext()){
			GghzRollerWeightEnum e = it.next();
			int w = v + e.getWeight();
			if(num >= v && num <= w){
				roller2 = e;
				break;
		...
		if ("NULL".equals(roller1.getIcon().getName())) {
			ws.add(new GghzWindowInfo(c, 1, 
					new GghzRollerInfo(roller1.getId(),roller1.getIndex(),null)));
		} else {
			ws.add(new GghzWindowInfo(c, 1, 
		...
		if ("NULL".equals(roller2.getIcon().getName())) {
			ws.add(new GghzWindowInfo(c, 2, 
					new GghzRollerInfo(roller2.getId(),roller2.getIndex(),null)));
		} else {
			ws.add(new GghzWindowInfo(c, 2, 
		...
		if ("NULL".equals(roller3.getIcon().getName())) {
			ws.add(new GghzWindowInfo(c, 3, 
					new GghzRollerInfo(roller3.getId(),roller3.getIndex(),null)));
		} else {
			ws.add(new GghzWindowInfo(c, 3, 
