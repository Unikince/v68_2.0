 ...
 * 轮子权重
 */
public enum AjxzRollerWeightEnum_backup {
	...
	private AjxzRollerWeightEnum_backup(int id, int index, AjxzIconEnum icon, int weight){
		this.id = id;
		this.index = index;
		this.icon = icon;
		this.weight = weight;
	...
	public static Collection<? extends Window> windowInfo(int c) {
		...
		List<AjxzRollerWeightEnum_backup> all = new ArrayList<AjxzRollerWeightEnum_backup>();
		for(AjxzRollerWeightEnum_backup e : values()){
			if(e.getId() == c){
				all.add(e);
			}
		}
		...
		/*List<WindowInfo> ws = new ArrayList<WindowInfo>();
		List<AjxzRollerWeightEnum> all = new ArrayList<AjxzRollerWeightEnum>();
		for(AjxzRollerWeightEnum e : values()){
			if(e.getId() == c){
				all.add(e);
			}
		}
		int total = 0;
		for(AjxzRollerWeightEnum e : all){
			total += e.getWeight();
		}
		int num = RandomUtil.getRandom(0, total);
		int v = 0;
		AjxzRollerWeightEnum roller = null;
		Iterator<AjxzRollerWeightEnum> it = all.iterator();
		while(it.hasNext()){
			AjxzRollerWeightEnum e = it.next();
			int w = v + e.getWeight();
			if(num >= v && num <= w){
				roller = e;
				break;
	...
	public static WindowInfo windowInfo(int c,int d) {
		...
		int random = RandomUtil.getRandom(0, all.size() - 1);
		ws.add(new WindowInfo(c, d, all.get(random).getIcon()));
