 ...
 * 轮子权重
 */
public enum YzhxRollerWeightEnum {
	...
	private YzhxRollerWeightEnum(int id, int index, YzhxIconEnum icon, int weight){
		this.id = id;
		this.index = index;
		this.icon = icon;
		this.weight = weight;
	 ...
	 * @return
	 */
	public static Collection<? extends Window> windowInfo(int c, List<YzhxOdds> all) {
		List<WindowInfo> ws = new ArrayList<WindowInfo>();
		List<YzhxOdds> col = new ArrayList<>();
		for (YzhxOdds odds : all) {
			if(odds.getCol() == c){
				col.add(odds);
			}
		}
		...
		for (int i = 1; i <= 3; i++) {
			YzhxIconEnum icon = null;
			YzhxOdds odds = col.get(random);
			//System.out.println("要对比的图标:" + odds);
			for (YzhxIconEnum e : YzhxIconEnum.values()) {
	 ...
	 * @return
	 */
	public static WindowInfo windowInfo(int c,int d,List<YzhxOdds> all) {
		List<WindowInfo> ws = new ArrayList<WindowInfo>();
		List<YzhxOdds> col = new ArrayList<>();
		for (YzhxOdds odds : all) {
			if(odds.getCol() == c){
		...
		YzhxIconEnum icon = null;
		for (YzhxIconEnum e : YzhxIconEnum.values()) {
			if (e.getName().equalsIgnoreCase(col.get(random).getName())) {
				icon = e;
			}
		}
		if (icon.isWild()) {
