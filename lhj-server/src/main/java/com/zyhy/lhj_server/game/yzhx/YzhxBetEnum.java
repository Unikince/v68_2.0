 ...
 *
 */
public enum YzhxBetEnum implements Bet{
	...
	private YzhxBetEnum(int id, double betcoin) {
		this.id = id;
	...
	public static YzhxBetEnum getById(int id) {
		for(YzhxBetEnum v : values()){
			if(v.getId() == id){
				return v;
			}
		}
	...
	public static YzhxBetEnum getByBetcoin(double jetton) {
		for(YzhxBetEnum v : values()){
