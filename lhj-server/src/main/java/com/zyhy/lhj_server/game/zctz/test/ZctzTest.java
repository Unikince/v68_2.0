package com.zyhy.lhj_server.game.zctz.test;
...
public class ZctzTest {
	...
	public  void initTest() throws IOException, InterruptedException {
		...
		String path = "C:\\Users\\Administrator\\Desktop\\shuzhi\\zctz\\1";
		String name = "zctz-" + format +".txt";
		File file = new File(path, name);
		System.out.println(file.createNewFile());
		for (int i = 1; i <= round; i++) {
			String testLogin1 = testLogin();
			ZctzResult result = testAuto();
	 ...
	 */
	public  ZctzResult testAuto() throws InterruptedException{
		// 统计结果
		ZctzResult yzhxResult = new ZctzResult();
		// 开始测试
		for (int i = 0; i < betCount; i++) {
			//Thread.sleep(20L);
			gameCount ++;
			System.out.println("目前自动运行次数: " + gameCount);
			ZctzGameBetResult result = testStartGame(line,lineBet);
			dataRecord(result);
	 ...
	 * @return 
	 */
	public  ZctzGameBetResult testStartGame(int line,double lineBet){
		int messageid = 1302;
		...
		map.put("uuid", uuid);
		mc.setParams(map);
		ZctzGameBetResult result = (ZctzGameBetResult) mainProcess.mainProcessMsg(mc);
		return result;
	}
	 ...
	 * @param yzhxResult 
	 */
	public  void dataRecord(ZctzGameBetResult result){
