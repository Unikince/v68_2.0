 ...
 *
 */
public enum GghzBetEnum implements Bet{
	a(0,0),
	b(1,0),
	c(2,0),
	d(3,0),
	...
	GghzBetEnum(int id, double betcoin){
		this.id = id;
	...
	public static List<GghzBetLimitInfo> infos() {
		List<GghzBetLimitInfo> is = new ArrayList<GghzBetLimitInfo>();
		for(GghzBetEnum b : values()) {
			is.add(new GghzBetLimitInfo(b));
		}
		return is;
	}

	public static GghzBetEnum getById(int id) {
		for(GghzBetEnum v : values()){
			if(v.getId() == id){
				return v;
			}
		}
	...
	public static GghzBetEnum getByBetcoin(double jetton) {
		for(GghzBetEnum v : values()){
