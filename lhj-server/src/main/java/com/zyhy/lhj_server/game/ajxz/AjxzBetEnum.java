 ...
 *
 */
public enum AjxzBetEnum implements Bet{
	...
	private AjxzBetEnum(int id, double betcoin) {
		this.id = id;
	...
	public static AjxzBetEnum getById(int id) {
		for(AjxzBetEnum v : values()){
			if(v.getId() == id){
				return v;
			}
		}
	...
	public static AjxzBetEnum getByBetcoin(double jetton) {
		for(AjxzBetEnum v : values()){
