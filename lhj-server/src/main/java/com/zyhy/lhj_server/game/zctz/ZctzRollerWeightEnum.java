 ...
 * 轮子权重
 */
public enum ZctzRollerWeightEnum {
	;
	...
	private ZctzRollerWeightEnum(int id, int index, ZctzIconEnum icon, int weight){
		this.id = id;
		this.index = index;
		this.icon = icon;
		this.weight = weight;
	 ...
	 * @return
	 */
	public static Collection<? extends Window> windowInfo(int c, List<ZctzOdds> all) {
		List<WindowInfo> ws = new ArrayList<WindowInfo>();
		List<ZctzOdds> col = new ArrayList<>();
		List<ZctzIconEnum> icons = new ArrayList<>();
		// 取出一列的图标
		for (ZctzOdds odds : all) {
			if(odds.getCol() == c){
				col.add(odds);
			}
		}
		//System.out.println("col" + col);
		for (ZctzOdds odds : col) {
			ZctzIconEnum icon = null;
		...
		for (int i = 1; i <= 3; i++) {
			ZctzIconEnum icon = icons.get(random);
