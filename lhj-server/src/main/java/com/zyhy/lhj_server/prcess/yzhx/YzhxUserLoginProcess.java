...
import com.zyhy.lhj_server.prcess.result.yzhx.YzxhLoginResult;
...
@Order
@Component
public class YzhxUserLoginProcess extends AbstractHttpMsgProcess{
	...
	public int getMessageId() {
		return MessageConstants.YZHX_USER_APPLY_LOGIN;
	}

	@Override
	public HttpMessageResult handler(String uuid, Map<String, String> body)
			throws Throwable {
		YzxhLoginResult result = new YzxhLoginResult();
		//获取用户信息
...
//		CommonLog.getInstance(restTemplate).createUserLhjLog(LogType.LOGIN_ZCTZ, ZctzConstants.SERVER_NAME, user.getRoleId(), user.getNickname(), result.getGold());
		
		UserLhjLog log = UserLhjLog.build(Constants.YZHX_SERVER_NAME, "login", roleid, user.getNickname(), user.getGold());
