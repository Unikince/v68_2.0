...
@Order
@Component
public class YzhxStartGameProcess extends AbstractHttpMsgProcess{
	@Autowired
	...
	public int getMessageId() {
		return MessageConstants.YZHX_START_GAME_INFO;
	}
	...
	public HttpMessageResult handler(String uuid, Map<String, String> body)
		...
		if (freeInfo != null && freeInfo.getFreeNum() > 0) {
			result.setRet(2);
			result.setMsg("免费游戏中");
			return result;
		}
...
//		userService.bet(roleid, betInfo.total(), roundId, uuid, Constants.BETTYPE1);
		...
		//免费信息
		if (result.isFree() && result.getFreeNum() > 0) {
			YzhxFreeInfo free = new YzhxFreeInfo();
			free.setBetInfo(betInfo);
			free.setFreeNum(result.getFreeNum());
			free.setGold(0);
			result.setFreeInfo(free);
			gameService.saveFreeInfo(roleid, free, userinfo, uuid);
		}
...
//		gameService.saveBigReward(Constants.YZHX_SERVER_NAME, betInfo, result.getRewardcoin(), roleid);
		...
		GameLhjLog log = GameLhjLog.build(Constants.YZHX_GAME_NAME, "start", roleid,userinfo.getNickname(),uuid, totalBet,result.getRewardcoin(),1,ispool,JSON.toJSONString(srr));
