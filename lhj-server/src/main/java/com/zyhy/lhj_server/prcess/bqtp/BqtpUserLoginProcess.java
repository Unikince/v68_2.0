...
import com.zyhy.lhj_server.prcess.result.bqtp.BqtpLoginResult;
...
@Order
@Component
public class BqtpUserLoginProcess extends AbstractHttpMsgProcess{
	...
	public int getMessageId() {
		return MessageConstants.BQTU_USER_APPLY_LOGIN;
	}

	@Override
	public HttpMessageResult handler(String uuid, Map<String, String> body)
			throws Throwable {
		BqtpLoginResult result = new BqtpLoginResult();
		//获取用户信息
...
//		CommonLog.getInstance(restTemplate).createUserLhjLog(LogType.LOGIN_CZDBZ, CzdbzConstants.SERVER_NAME, user.getRoleId(), user.getNickname(), result.getGold());
		UserLhjLog log = UserLhjLog.build(Constants.BQTP_SERVER_NAME, "login", roleid, user.getNickname(), user.getGold());
