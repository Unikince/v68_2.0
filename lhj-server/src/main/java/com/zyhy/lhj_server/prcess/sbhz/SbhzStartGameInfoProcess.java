...
@Order
@Component
public class SbhzStartGameInfoProcess extends AbstractHttpMsgProcess{
	@Autowired
	private BgManagementServiceImp bgManagementServiceImp;
	@Autowired
	private SbhzOpenGamePanlProcess openGamePanlProcess;
	@Autowired
	...
	public int getMessageId() {
		return MessageConstants.SBHZ_START_GAME_INFO;
	}
...
//		int betnum = Integer.parseInt(body.get("num"));//级别 1-9
		...
		if(!betEnum || betnum < 1 || betnum > SbhzWinLineEnum.values().length){
			result.setRet(2);
			result.setMsg("投注错误");
			return result;
		}
...
//		userService.bet(roleid, betInfo.total(), roundId, uuid, Constants.BETTYPE1);
		...
		if ((p.getStatus() == JackPool.STATUS_KILL && result.getRewardcoin() > 0)
				|| (p.getStatus() == JackPool.STATUS_KILL && result.isTurned())
				|| (p.getStatus() == JackPool.STATUS_KILL && result.isMonkey())) {
		...
		// 重转信息
		if(result.isTurned()){
			SbhzWildMonKey monkey = result.getMonkey();
			gameService.save(monkey, roleid, uuid);
		}
...
//		CommonLog.getInstance(restTemplate).createGameLhjLog(LogType.GAME_CZDBZ, CzdbzConstants.SERVER_NAME, userinfo.getRoleId(), betcoin + "&" + betnum + "&" + JSON.toJSONString(result));
		GameLhjLog log = GameLhjLog.build(Constants.SBHZ_GAME_NAME, "start", roleid,userinfo.getNickname(),uuid, m,result.getRewardcoin(),1,ispool,JSON.toJSONString(srr));
