...
import com.zyhy.lhj_server.service.gghz.GghzGameService;
...
@Order
@Component
public class GghzStartGameInfoProcess extends AbstractHttpMsgProcess{
	@Autowired
	private BgManagementServiceImp bgManagementServiceImp;
	@Autowired
	private GghzOpenGamePanlProcess openGamePanlProcess;
	@Autowired
	private GghzGameService gameService;
	@Autowired
	...
	private static final Logger LOG = LoggerFactory.getLogger(Constants.GGHZ_SERVER_NAME);
	@Override
	public int getMessageId() {
		return MessageConstants.GGHZ_START_GAME_INFO;
	}
	...
	public HttpMessageResult handler(String uuid, Map<String, String> body)
			throws Throwable {
		// 返回信息
		GghzGameBetResult result = new GghzGameBetResult();
		String roleid = body.get("roleid");
		double betcoin = Double.parseDouble(body.get("jetton"));//档位
		int betnum = Integer.parseInt(body.get("num"));//级别 1，2，3
		...
		//总投注
		double m = new BigDecimal(betcoin * betnum).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
		// 检查游戏币合理性
		if (usercoin < m) {
			result.setRet(4);
			result.setMsg("游戏币不足");
		...
		String roundId = MessageIdEnum.HYHZ.getGameId() + "|" + userService.getOrderId();
//		userService.bet(roleid, betinfo.total(), roundId, uuid, Constants.BETTYPE1);
		
		// 查询当前赔率
		SoltGameInfo gameInfo = bgManagementServiceImp.queryGameInfo(MessageIdEnum.HYHZ.getRedisName());
...
//		userService.payout(roleid, result.getRewardcoin(), roundId, uuid);
		...
		// 记录日志
		//gameService.saveBigReward(Constants.GGHZ_SERVER_NAME, betinfo, result.getRewardcoin(), roleid);
		ShowRecordResult srr = new ShowRecordResult();
		int ispool = 0;
		if (oddsReward > 0) {
			ispool = 1;
		}
		srr.setGameName(Constants.GGHZ_GAME_NAME);
...
//		CommonLog.getInstance(restTemplate).createGameLhjLog(LogType.GAME_GGHZ, Constants.SERVER_NAME, roleid, betcoin + "&" + betnum + "&" + JSON.toJSONString(result));
		GameLhjLog log = GameLhjLog.build(Constants.GGHZ_GAME_NAME, "start", roleid,userinfo.getNickname(),uuid,m,result.getRewardcoin(),1,ispool, JSON.toJSONString(srr));
