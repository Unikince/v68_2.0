...
@Order
@Component
public class GghzUserLoginProcess extends AbstractHttpMsgProcess{
	...
	public int getMessageId() {
		return MessageConstants.GGHZ_USER_APPLY_LOGIN;
	}

	@Override
	public HttpMessageResult handler(String uuid, Map<String, String> body)
			throws Throwable {
//		playerService.checkPlayer("aaa");
		...
		result.setGold(user.getGold());
//		CommonLog.getInstance(restTemplate).createUserLhjLog(LogType.LOGIN_GGHZ, Constants.SERVER_NAME, user.getRoleId(), user.getNickname(), result.getGold());
		UserLhjLog log = UserLhjLog.build(Constants.GGHZ_SERVER_NAME, "login", roleid, user.getNickname(), user.getGold());
