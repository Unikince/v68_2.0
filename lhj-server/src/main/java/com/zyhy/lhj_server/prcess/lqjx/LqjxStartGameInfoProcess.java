...
@Order
@Component
public class LqjxStartGameInfoProcess extends AbstractHttpMsgProcess{
	@Autowired
	private BgManagementServiceImp bgManagementServiceImp;
	@Autowired
	private LqjxOpenGamePanlProcess openGamePanlProcess;
	@Autowired
	...
	public int getMessageId() {
		return MessageConstants.LQJX_START_GAME_INFO;
	}
	...
	public HttpMessageResult handler(String uuid, Map<String, String> body)
		...
		int betnum = Integer.parseInt(body.get("num"));// 下注硬币档位
		...
		double bet = NumberTool.multiply(lineBet, 50).doubleValue();
		// 用户信息
...
//			userService.bet(roleid, 0, roundId, uuid, Constants.BETTYPE0);
		...
		if(result.isReplenish()){
			LqjxReplenish newRep = result.getRep();
...
//		userService.payout(roleid, result.getRewardcoin(), roundId, uuid);
		...
		// 记录日志
		//gameService.saveBigReward(Constants.LQJX_SERVER_NAME, betInfo, result.getRewardcoin(), roleid);
		...
		GameLhjLog log = GameLhjLog.build(Constants.LQJX_GAME_NAME, "start", roleid,userinfo.getNickname(),uuid,bet1,payout,type,ispool, JSON.toJSONString(srr));
