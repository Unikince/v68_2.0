...
@Order
@Component
public class SbhzUserLoginProcess extends AbstractHttpMsgProcess{
	...
	public int getMessageId() {
		return MessageConstants.SBHZ_USER_APPLY_LOGIN;
	}
...
//		CommonLog.getInstance(restTemplate).createUserLhjLog(LogType.LOGIN_CZDBZ, CzdbzConstants.SERVER_NAME, user.getRoleId(), user.getNickname(), result.getGold());
		UserLhjLog log = UserLhjLog.build(Constants.SBHZ_SERVER_NAME, "login", roleid, user.getNickname(), user.getGold());
