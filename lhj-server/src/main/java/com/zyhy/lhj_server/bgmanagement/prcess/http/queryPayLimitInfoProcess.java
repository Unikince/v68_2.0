...
@Order
//@Component
public class queryPayLimitInfoProcess extends AbstractHttpMsgProcess{
	@Autowired
	private BgManagementDaoImp bgManagementDaoImp;
	
	@Override
	public int getMessageId() {
		return SoltMessageConstants.QUERYPAYLIMITINFO;
	}
	...
	public HttpMessageResult handler(String uuid, Map<String, String> body)
		...
		List<PayoutLimit> query = bgManagementDaoImp.queryPayLimitInfo();
		if (query.size() > 0) {
			result.setMsg(JSONObject.toJSONString(query));
		} else {
			result.setRet(2);
			result.setMsg("queryPayLimitInfo is empty!");
		}
		return result;
	}
